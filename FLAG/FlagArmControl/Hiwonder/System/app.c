...
uint8_t up_message[] = {0xAA, 0x01, 0x00};
uint8_t down_message[] = {0xAA, 0x01, 0x01};
uint8_t default_message[] = {0xAA, 0x00, 0x00};
//机械爪控制函数
void crew_control(char msg);
...
void crew_control(char msg)
{
	switch(msg) 
	{
			case 'l': 
			{ //正方形
				serial_servo_set_position(1, 450, 1000);
				break;
			}
			case 'p': 
			{ //圆
				serial_servo_set_position(1, 650, 1000);
				break;
			}
	}
}